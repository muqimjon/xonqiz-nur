...
@Component({
  ...
  styles: [`
    ...
    .map-layout { display: grid; grid-template-columns: 340px 1fr; gap: 24px; align-items: start; }
    ...
    .loc-icon { background: rgba(245,166,35,0.1); color: var(--color-accent); border: 1px solid rgba(245,166,35,0.2); }
    .map-icon { background: rgba(74,222,128,0.1); color: #4ade80; border: 1px solid rgba(74,222,128,0.2); }
    .time-icon { background: rgba(79,195,247,0.1); color: var(--color-electric); border: 1px solid rgba(79,195,247,0.2); }

    .ic-label { display: block; font-size: 0.72rem; font-weight: 700; color: var(--color-text-muted); letter-spacing: 0.05em; text-transform: uppercase; margin-bottom: 3px; }
    .ic-val { display: block; font-size: 0.9rem; font-weight: 600; color: var(--color-text); }

    .navigate-btn { width: 100%; justify-content: center; margin-top: 8px; }

    .map-frame-wrap { height: 420px; }
    .map-frame { height: 100%; padding: 6px; overflow: hidden; }
...
export class MapComponent {
  ...
  get mapSrc(): SafeResourceUrl {
    const url = 'https://yandex.uz/map-widget/v1/?ll=71.520984%2C40.291645&z=16&l=map&pt=71.520984,40.291645,pm2rdl';

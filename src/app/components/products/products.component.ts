...
@Component({
  ...
  styles: [`
    .orb-bg {
      ...
      top: -200px; right: -200px; pointer-events: none; filter: blur(60px);
    }
    ...
    .product-col { padding: 36px; display: flex; flex-direction: column; gap: 28px; }

    .col-header { display: flex; align-items: center; gap: 16px; }
    .col-icon {
      width: 52px; height: 52px; border-radius: 14px;
      display: flex; align-items: center; justify-content: center; flex-shrink: 0;
    }
    .electric-icon { background: rgba(245,166,35,0.12); color: var(--color-accent); border: 1px solid rgba(245,166,35,0.2); }
    .water-icon { background: rgba(79,195,247,0.1); color: var(--color-electric); border: 1px solid rgba(79,195,247,0.2); }
    ...
    .product-item {
      ...
      padding: 11px 0; font-size: 0.93rem; color: var(--color-text-muted);
      border-bottom: 1px solid var(--color-border);
      transition: color 0.2s ease;
    }
    .product-item:last-child { border-bottom: none; }
    .product-item:hover { color: var(--color-text); }
    .item-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
    .electric-dot { background: var(--color-accent); }
    .water-dot { background: var(--color-electric); }

    .col-footer {
      padding: 12px 16px; border-radius: 10px;
      font-size: 0.82rem; font-weight: 600;
    }
    .electric-footer { background: rgba(245,166,35,0.07); color: var(--color-accent); }
    .water-footer { background: rgba(79,195,247,0.07); color: var(--color-electric); }

    @media (max-width: 768px) {
      .products-grid { grid-template-columns: 1fr; }
      .product-col { padding: 24px; }
    }
  `]
})
